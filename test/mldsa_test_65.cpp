...
#define ML_DSA_PRIVATE_KEY(x) std::vector<uint8_t> x(sizeof(pqc_ml_dsa_private_key_65))
#define ML_DSA_PUBLIC_KEY(x) std::vector<uint8_t> x(sizeof(pqc_ml_dsa_public_key_65))
#define ML_DSA_SIGNATURE(x) std::vector<uint8_t> x(sizeof(pqc_ml_dsa_signature_65))


TEST(ML_DSA_65, ACVP_KAT_FROM_JSON)
{
    static const std::filesystem::path current(__FILE__);
    static const auto base_path = current.parent_path() / "mldsa";
    static const auto keygen_responses_path = base_path / "ml-dsa-65-keygen.rsp";
    static const auto siggen_responses_path = base_path / "ml-dsa-65-siggen-hedged.rsp";
    static const auto sigver_responses_path = base_path / "ml-dsa-65-sigver.rsp";
    ...
    EXPECT_TRUE(expected == "# ML-DSA-65-FROM-JSON");
    ...
    for (size_t i = 0; i < 25; ++i)
    {
        ...
        CIPHER_HANDLE alice =
            PQC_context_init_asymmetric(PQC_CIPHER_ML_DSA_65, nullptr, 0, kat_sk.data(), kat_sk.size());
    ...
    for (size_t i = 0; i < 15; ++i)
    {
        ...
        CIPHER_HANDLE context =
            PQC_context_init_asymmetric(PQC_CIPHER_ML_DSA_65, kat_pk.data(), kat_pk.size(), nullptr, 0);
