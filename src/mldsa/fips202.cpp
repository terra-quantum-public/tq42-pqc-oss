 ...
 **************************************************/
static unsigned int keccak_absorb(uint64_t s[25], unsigned int r, unsigned int pos, const uint8_t * m, size_t mlen)
{
    unsigned int i;
    uint8_t t[8] = {0};
    ...
    if (mlen)
    {
        ...
        for (i = 0; i < mlen; i++)
            t[i] = m[i];
        s[pos / 8] ^= load64(t);
        pos += static_cast<unsigned int>(mlen);
    }
 ...
 * Returns new position pos in current block
 **************************************************/
static unsigned int keccak_squeeze(uint8_t * out, size_t outlen, uint64_t s[25], unsigned int r, unsigned int pos)
{
    unsigned int i;
    uint8_t t[8];
    ...
    for (i = 0; i < outlen; i++)
        out[i] = t[i];
    pos += static_cast<unsigned int>(outlen);
    return pos;
}
