...
std::unique_ptr<PQC_Context> FalconFactory::create_context(const ConstBufferView & private_key) const
{
    if (private_key.size() != PQC_FALCON_PRIVATE_KEYLEN)
    {
        throw BadLength();
    }
    return std::make_unique<FalconContext>(reinterpret_cast<const pqc_falcon_private_key *>(private_key.const_data()));
}
...
void FalconFactory::generate_keypair(const BufferView & public_key, const BufferView & private_key) const
{

    if (private_key.size() != PQC_FALCON_PRIVATE_KEYLEN || public_key.size() != PQC_FALCON_PUBLIC_KEYLEN)
    {
        throw BadLength();
    }
    else
    {
        ...
        randombytes(BufferView(&seed, sizeof(seed)));
        shake_256_init_prng_from_seed(&ctx, ConstBufferView(&seed, sizeof(seed)));

        int result = 0;
        size_t pubkey_size = public_key.size();
        ...
        if (private_key.size() < PQC_FALCON_PRIVKEY_SIZE(10) ||
            (public_key.data() != NULL && pubkey_size < PQC_FALCON_PUBKEY_SIZE(10)))
        {
...
) const
{
    ...
    if (public_key.size() != PQC_FALCON_PUBLIC_KEYLEN)
    {
        throw BadLength();
    }
...
void FalconContext::sign(const ConstBufferView & buffer, const BufferView & signature) const
{
    if (signature.size() != PQC_FALCON_SIGNATURE_LEN)
    {
        throw BadLength();
    ...
    int result = falcon_sign_dyn_finish(
        signature, FALCON_SIG_PADDED, ConstBufferView(private_key_.private_key, PQC_FALCON_PRIVATE_KEYLEN), &c, nonce,
        ConstBufferView(tmp, PQC_FALCON_TMPSIZE_SIGNDYN(10))
    );
