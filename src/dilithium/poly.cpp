...
void polyt1_pack(uint8_t * r, const poly * a)
{
    ...
    for (i = 0; i < N / 4; ++i)
    {
        r[5 * i + 0] = static_cast<uint8_t>(a->coeffs[4 * i + 0] >> 0);
        r[5 * i + 1] = static_cast<uint8_t>((a->coeffs[4 * i + 0] >> 8) | (a->coeffs[4 * i + 1] << 2));
        r[5 * i + 2] = static_cast<uint8_t>((a->coeffs[4 * i + 1] >> 6) | (a->coeffs[4 * i + 2] << 4));
        r[5 * i + 3] = static_cast<uint8_t>((a->coeffs[4 * i + 2] >> 4) | (a->coeffs[4 * i + 3] << 6));
        r[5 * i + 4] = static_cast<uint8_t>(a->coeffs[4 * i + 3] >> 2);
    }
}
