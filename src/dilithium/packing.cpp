...
)
{
    ...
    for (i = 0; i < L; ++i)
        polyeta_unpack(&s1[i], sk + i * POLYETA_PACKEDBYTES);
    sk += L * POLYETA_PACKEDBYTES;

    for (i = 0; i < K; ++i)
        polyeta_unpack(&s2[i], sk + i * POLYETA_PACKEDBYTES);
    sk += K * POLYETA_PACKEDBYTES;
...
void pack_sig(uint8_t sig[CRYPTO_BYTES], const uint8_t c[SEEDBYTES], const poly * z, const poly * h)
{
    ...
    for (i = 0; i < L; ++i)
        polyz_pack(sig + i * POLYZ_PACKEDBYTES, &z[i]);
    ...
    for (i = 0; i < K; ++i)
    {
        for (j = 0; j < N; ++j)
            if (h[i].coeffs[j] != 0)
                sig[k++] = static_cast<uint8_t>(j);

        sig[OMEGA + i] = static_cast<uint8_t>(k);
    }
}
